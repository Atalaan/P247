...
- CWD fixed: {{CWD}}; to run elsewhere: cd C:\\path && cmd in **one** command; no ~ or $HOME.
- On Windows/cmd.exe, never use Unix-only shell syntax such as mkdir -p, rm -rf, touch, cat, cp, mv, ls, chmod, grep, or here-docs.
- For creating files, prefer write_to_file directly; do not run mkdir first unless a tool result proves the parent directory is missing.
- For forced directory creation on Windows, use powershell -NoProfile -Command "New-Item -ItemType Directory -Force -Path 'path' | Out-Null".
- For Windows python -c commands, avoid nested JSON key quotes; use dict(key=value) or write_to_file a .py script, then run python script.py.
