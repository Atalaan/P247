...
const runtimeMetadataFromApiReq = (message: ClineMessage): Record<string, unknown> | undefined => {
...
export const findLocalRuntimeForMessage = (
	message: ClineMessage,
	messages: ClineMessage[],
): Record<string, unknown> | undefined => {
	const direct = runtimeMetadataFromApiReq(message)
	if (direct) {
		return direct
	}
	...
	for (let i = startIndex; i >= 0; i--) {
		const metadata = runtimeMetadataFromApiReq(messages[i])
		if (metadata) {
			return metadata
		}
	}
...
export const findLocalRuntimeMetadataForMessage = (
	...
	return formatLocalRuntimeMetadata(findLocalRuntimeForMessage(message, messages))
}
